...
def find_password():
    ...
    email = ""
    password = ""
    for i in password_data.password_data:
        if password_data.decoder(i) == website.encode('utf-8'):
            email = password_data.decoder(password_data.password_data[i][0])
            password = password_data.decoder(password_data.password_data[i][1])
            pyperclip.copy(password.decode('utf-8'))
            messagebox.showinfo(title=website,
                                message=f"Email: {email}\nPassword: {password}\nYour password for {website} was also copied to clipboard.")
    if email == "":
        messagebox.showinfo(title="Error", message=f"No data for '{website}' website was found!")
